    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn('group/sidebar-wrapper h-full flex w-full', className)}
          {...props}>
          {children}
        </div>
      </TooltipProvider>
...
function SidebarHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      data-sidebar="header"
      className={cn(
        'flex flex-col gap-2 p-2 transition-[border] duration-200 border-b-2! border-l-2 border-l-transparent group-data-[state=collapsed]:border-l-border',
        className
      )}
      {...props}
    />
